...
6> .steam.on | off 开启 | 关闭 Steam群播报
7> .steam.update [name] [pic] 更新群组名称和头像
8> .steam.call [name] 设置玩家昵称
==========================="""
...
async def unbind_handle(event: Event, target: Target = Depends(get_target)):
    ...
    if bind_data.get(parent_id, user_id) is not None:
        ...
        await unbind.finish("未绑定steam_id，无中生有哈^o^")
...
async def info_handle(
    ...
    if len(at) != 0:
        ...
        if user_data is None:
            await info.finish(
                "未绑定steam_id, 请使用: .steam.bind 进行绑定后重试......"
            )
...
async def disable_handle(target: Target = Depends(get_target)):
    ...
    await disable.finish("已禁用 Steam 播报 o.O")
...
async def set_nickname_handle(
    ...
    if user_data is None:
        await set_nickname.finish(
            "未绑定steam_id，请先使用[.steam.bind]绑定后再设置昵称哦~"
        )
