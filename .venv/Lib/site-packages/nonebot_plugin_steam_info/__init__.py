...
help = on_command("steam.help", aliases={"steam.menu"}, priority=10)
bind = on_command("steam.bind", aliases={"steam.add"}, priority=10)
unbind = on_command("steam.unbind", aliases={"steam.del"}, priority=10)
info = on_command("steam.info", aliases={'steam.profile'}, priority=10)
check = on_command("steam.check", aliases={'steam.show', 'steam.group'}, priority=10)
enable = on_command("steam.enable", aliases={"steam.on"}, priority=10, permission=SUPERUSER)
disable = on_command("steam.disable", aliases={"steam.off"}, priority=10, permission=SUPERUSER)
status = on_command("steam.status", aliases={"steam.onwatch"}, priority=5)
update_parent_info = on_command("steam.update", aliases={"steam.change"}, priority=10, permission=SUPERUSER)
set_nickname = on_command("steam.nickname", aliases={"steam.call"}, priority=10)
...
async def broadcast_steam_info(
    ...
    for entry in play_data:
        ...
        if entry["type"] == "start":
            ...
            msg.append(
                f"{player['personaname']} 玩了{time_str} {old_player['gameextrainfo']} 之后去做别的事情了。"
            )
        elif entry["type"] == "change":
            msg.append(
                f"{player['personaname']} 转移了阵地！从 {old_player['gameextrainfo']} 变成了 {player['gameextrainfo']}。"
            )
...
async def help_handle():
    message = """==========<R-I-F-T>==========
1> .steam.help 请帮帮我！
2> .steam.bind [] 添加steam_id或好友代码
3> .steam.unbind [] 删除已绑定的steam_id
4> .steam.info [ ~ | @ta | steam_id] 查看个人主页
5> .steam.show 看看朋友们都在steam上做什么吧~
6> .steam.on | off 开启 | 关闭 Steam播报
7>. steam.status 查看自己的播报状态
8> .steam.update [name] [pic] 更新群组名称和头像
9> .steam.call [name] 设置玩家昵称
==========<R-I-F-T>=========="""
...
async def bind_handle(
    ...
    if not arg.isdigit():
        await bind.finish(
            "请输入正确的steam_id或好友代码。格式: .steam.bind [steam_id | friend_code]"
        )
    ...
    if user_data := bind_data.get(parent_id, event.get_user_id()):
        ...
        await bind.finish(f"已更新你的steam_id为[{steam_id}]。啊嘞？之前那个是谁？")
    else:
        bind_data.add(
            parent_id,
        ...
        await bind.finish(f"已绑定你的steam_id为[{steam_id}]，欢迎加入~")
...
async def unbind_handle(event: Event, target: Target = Depends(get_target)):
    ...
    if bind_data.get(parent_id, user_id) is not None:
        ...
        await unbind.finish("已解绑steam_id，以后再见~")
    else:
        await unbind.finish("未绑定steam_id，无中生有哈 O.o")
...
async def info_handle(
    ...
    if len(at) != 0:
        ...
        if user_data is None:
            await info.finish("该用户未绑定steam_id，没看见人啊~")
...
async def update_parent_info_handle(
    ...
    if "avatar" not in info or "name" not in info:
        await update_parent_info.finish("文本中应包含图片和文字， 请补充参数！")

    parent_data.update(target.parent_id or target.id, info["avatar"], info["name"])
    await update_parent_info.finish("Steam群组信息更新成功！")
...
async def enable_handle(target: Target = Depends(get_target)):
    ...
    await enable.finish("已启用 Steam 播报 ^u^")
...
async def disable_handle(target: Target = Depends(get_target)):
    ...
    await disable.finish("已禁用 Steam 播报 -_-")


@status.handle()
async def status_hande(target: Target = Depends(get_target)):
    parent_id = target.parent_id or target.id
...
async def set_nickname_handle(
    ...
    if nickname == "":
        await set_nickname.finish("请输入昵称。格式: .steam.call [name]")

    user_data = bind_data.get(parent_id, event.get_user_id())

    if user_data is None:
        await set_nickname.finish(
            "未绑定steam_id，请先绑定后再设置昵称哦~"
        )

    user_data["nickname"] = nickname
    bind_data.save()

    await set_nickname.finish(f"已设置你的昵称为[{nickname}]，将在Steam播报中显示~")
