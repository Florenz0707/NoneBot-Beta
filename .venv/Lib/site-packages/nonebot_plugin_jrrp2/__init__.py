...
# 查询历史数据
def select_tb_all(qqid: str):
    cursor.execute("select * from jdata where QQID = ?", (qqid,))
    return cursor.fetchall()


# 查询今日是否存在数据
def select_tb_today(qqid: str, date: str):
    cursor.execute("select *from jdata where QQID = ? and Date = ?", (qqid, date))
...
# 判断是否本月
def same_month(date_string: str):
    d1 = datetime.datetime.strptime(date_string, '%y%m%d')
    d2 = datetime.datetime.today()
...
special_dict = {
    "2862333280": ("哇！qaq你好呀！", 100, 100),
    "2559815776": ("是...Master！", 30, 100),
    "3503218428": ("", 0, 20)
}
...
def generate_luck(bot: Bot, event: Event, cheat: bool):
    ...
    if res == -1:
        rand = random.Random()
        rand.seed(int(_date) + int(event.get_user_id()) - random.randint(0, 65535))
        bottom, top = 0, 100
        if _date == "241214":
            bottom = 95
        if event.get_user_id() in special_dict.keys():
            info = special_dict[event.get_user_id()][1]
            bottom, top = info[1], info[2]
        res = rand.randint(bottom, top)
        if cheat:
            res = int(res**0.5 * 10)
...
async def jrrp_handle(bot: Bot, event: Event):
    ...
    if event.get_user_id() in special_dict.keys():
        sp_data = special_dict[event.get_user_id()]
        if len(sp_data[0]) != 0:
            await jrrp.send(sp_data[0])
