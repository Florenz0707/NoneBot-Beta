...
special_dict = {
    "2862333280": ("哇！qaq你好呀！", 100),
    "2559815776": ("是...Master！", 30)
}
...
def generate_luck(bot: Bot, event: Event, cheat: bool):
    ...
    if res == -1:
        rand = random.Random()
        rand.seed(int(_date) + int(event.get_user_id()))
        bottom = 0
        if _date == "241214":
            bottom = 95
        if event.get_user_id() in special_dict.keys():
            bottom = special_dict[event.get_user_id()][1]
        res = min(rand.randint(bottom, 100) + random.randint(0, 10), 100)
        if cheat:
            res = int(res**0.5 * 10)
